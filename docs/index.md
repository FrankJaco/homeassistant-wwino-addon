...
Install the Wonderful Wino Add-on
...
Click on your User Account (bottom of the Home Assistant sidebar on the left).

Select the Security tab at the top of the screen and scroll to its bottom.

In the Long-lived access tokens section and click Create Token.

Name it WWino (or anything else you want) and click OK

Copy and paste it to the Configuration tab's HA_LONG_LIVED_TOKEN textbox.
 ...
 * Running on [http://172.30.33.13:5000](http://172.30.33.13:5000)
...
To get the most out of Wonderful Wino, the Local ToDo list integration, and a functioning Home Assistant Voice Assistant enhanced with Ai are required. (fyi, I personally use the Google Gemini integration and it does a nice job.) This section will tie these various parts together.

Local ToDo list:

If you have not done so already, install the Local ToDo list integration now.
...
For connectivity between Wonderful Wino and Home Assistant / Local ToDo list a small addition to the configuration.yaml file is required. You can use the FileEditor or VSCode add-ons for this task.
...
Sortable Wine List in a ToDo list card

Direct access to the full Wonderful Wino GUI

The Wine Rating interface (that is used by the automation above)

Badge with the total of "Unique Wines" on hand. (ignores the quantity of each)
...
Navigate to Settings > Add-ons and select Wonderful Wino.

Click the Open Web UI button.

Once the add-on's interface has loaded, copy the full URL from your browser's address bar.
...
You may want to consider adding the Samba Share add-on if you don't already have it installed.
...
Manually via the Wonderful Wino GUI

Vivino-Assisted via the Wonderful Wino GUI

Vivino-Assisted via Chrome Browser Extension

Vivino-Assisted via Android Helper App
...
Clicking the 🗑️Waste Basket deletes this wine completely from your inventory both on hand and in your history. You will have the opportunity to cancel the action.

Clicking ✏️Pencil provides a window to edit all details of the wine as well it can be used to do a "Save As" of your wine, say you bought 5 bottles and didn't realize a few of them were another vintage.
...
The button top-right (currently "reset" in the image) has double duty. It can be used set the default tiers (which are currently depicted). If changes were made to the cost tiers, the button performs a reset function which allows you to revert to your previous values.
