...
def scan_wine():
    ...
    cost_tier = data.get('cost_tier')  # Optional cost tier from payload
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        cursor.execute(
            "SELECT * FROM wines WHERE name = ? AND vintage IS ?",
            (wine_data['name'], wine_data['vintage'])
        )
        ...
        if existing_wine_row:
            # Match found! Update the existing record and use its URL as canonical
            existing_wine_dict = dict(existing_wine_row)  # safe: row_factory is sqlite3.Row
            logger.info(
                f"Found existing wine '{wine_data['name']}' ({wine_data['vintage']}) "
                f"with URL {existing_wine_dict['vivino_url']}. Consolidating under this canonical URL."
            )
            canonical_url_for_update = existing_wine_dict['vivino_url']
            wine_data['vivino_url'] = canonical_url_for_update  # overwrite scraped URL with canonical

    # Step 3: Insert or Update the database (pass cost_tier through)
    if insert_wine_data(wine_data, quantity, cost_tier):
        # Read back the full row so we can return it and sync HA To-Do
        updated_wine_dict = None
        with sqlite3.connect(DB_PATH) as conn_post_insert:
            conn_post_insert.row_factory = sqlite3.Row  # <-- FIX: ensure Row objects
            cursor_post_insert = conn_post_insert.cursor()
            cursor_post_insert.execute(
                "SELECT * FROM wines WHERE vivino_url = ?",
                (canonical_url_for_update,)
            )
            updated_wine_row = cursor_post_insert.fetchone()

            if updated_wine_row:
                # --- FIX: robust conversion no matter what type comes back ---
                try:
                    # sqlite3.Row (works when row_factory is set)
                    updated_wine_dict = dict(updated_wine_row)
                except TypeError:
                    # Fallbacks (shouldn't hit with row_factory set, but safe to keep)
                    try:
                        updated_wine_dict = dict(updated_wine_row._mapping)  # SQLAlchemy Row-style
                    except Exception:
                        cols = [c[0] for c in cursor_post_insert.description] if cursor_post_insert.description else []
                        updated_wine_dict = dict(zip(cols, updated_wine_row)) if cols else {}

        if updated_wine_dict:
            ...
            return jsonify({
                ...
                "wine_name": updated_wine_dict.get('name'),
                "vintage": updated_wine_dict.get('vintage'),
