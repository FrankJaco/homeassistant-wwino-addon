...
def _perform_scrape_attempt(url: str):
    ...
    # --- MODIFIED SECTION: Use a requests.Session() object ---
    # A Session object persists cookies and connection info, appearing more like a real user.
        ...
        try:
            ...
            for attempt in range(4): 
                if attempt > 0:
                    ...
                    time.sleep(delay)
                    # Update header with a new user-agent for the retry attempt
                    session.headers.update({ 'User-Agent': random.choice(user_agents) })
